...
class p_master
{
	...
	*/
	function module_auth($module_auth, $forum_id = false)
	{
		...
		if (method_exists($this, 'module_auth_local'))
		{
			return $this->module_auth_local($forum_id);
		}
		...
		static $parser = false;
		if (empty($parser)) $parser = new ModuleAuthParser();
 ...
 * This is probably slower than eval(), but makes eval() unecessary.
 *
 * @author APTX
 **/
class ModuleAuthParser
{
	...
	function _variable_callback($var_name)
	{
		...
		switch ($prefix)
		{
			...
			case 'requ':
				return !empty($_REQUEST[substr($var_name, 8)]);
